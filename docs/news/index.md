...
- Mobile stylesheet leaves body typography to the user theme instead of
  shrinking it.
- Tabs and nav links keep the tap-target floor at narrow widths.

## a11yviz 0.1.7

- Theme keeps title, axis title, and legend at the body floor while axis
  tick text sits two points smaller.
...
- First release: accessible themes, palettes, and audits for ggplot2,
  plotly, and Quarto.
- WCAG-tagged palettes with a per-criterion audit, reference rubric, and
  bundled stylesheet.
- Interactive playground ships as a static page inside the docs site —
  no install needed.
