...
plotnine
pandas
https://mshin77.github.io/a11yviz/articles/wheels/a11yviz-0.1.2-py3-none-any.whl
...
def server(input, output, session):
    ...
    @render.data_frame
    def audit_before():
        rows = a11y_audit(base_plot(), level=input.level())
        return render.DataGrid(pd.DataFrame(rows), height="320px")

    @render.data_frame
    def audit_after():
        rows = a11y_audit(improved_plot(input.level()), level=input.level())
        return render.DataGrid(pd.DataFrame(rows), height="320px")
