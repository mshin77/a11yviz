...
Five accessibility wins from a few lines: `shape = species` encodes
group via marker (Success Criterion 1.4.1),
`scale_color_a11y("dark2_8")` swaps in colors that clear 3:1 on white
(Success Criterion 1.4.11; ggplot’s default scale fails here for some
pairs), `theme_a11y("AA")` drops the grey panel background and sets
AA-compliant fonts (Success Criterion 1.4.3),
[`labs()`](https://ggplot2.tidyverse.org/reference/labs.html) names the
axes (Success Criterion 2.4.6), and
